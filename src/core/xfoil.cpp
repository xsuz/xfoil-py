...
bool XFoil::iblpan()
{
	...
	if(iblmax>IVX)
	{
		...
		std::stringstream ss;
		ss << "Current chordline angle: " << iblmax << "\nproceeding anyway...";
...
bool XFoil::setbl()
{
	...
	for(is=1;is<=2;is++)
	{
		...
		if(tforce[is])
		{
			strOut << "     Side " << is << ",  forced  transition at x/c = " << xoctr[is] <<" " << itran[is] <<"\n";
			//TRACE(strOut);
			writeString(strOut.str());

		}
		else
		{
			strOut << "     Side " << is << ",  free  transition at x/c = " << xoctr[is] <<" " << itran[is] <<"\n";
			//TRACE(strOut);
			writeString(strOut.str());
		}
...
bool XFoil::ViscousIter()
{
	...
	if(rlx<1.0)
	{
		ss << "     rms:"<< rmsbl <<"   max:" << rmxbl << " at " << imxbl << " " << ismxbl <<"   rlx:" << rlx << "\n";
		str=ss.str();
	}
	else if(fabs(rlx-1.0)<0.001)
	{
		ss << "     rms:"<< rmsbl <<"   max:" << rmxbl << " at " << imxbl << " " << ismxbl << "\n";
		str=ss.str();
	}
		...
		double xcir[], double ycir[], double scir[], double qcir[])
{
	...
	*ncir = nc;
	...
	std::stringstream ss;
	ss << "qccalc: cl convergence failed.  dalpha =" <<dalfa;
	writeString(ss.str());
}
